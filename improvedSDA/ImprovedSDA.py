...
from sklearn.metrics import auc, recall_score, matthews_corrcoef, roc_auc_score
...
def get_skewed_F_measure(X1, X2, h1, h2, alpha):
    ...
    MCC= all / (div**0.5)
    skewedF1 = (1 + alpha) * P * Recall / (alpha * P + Recall)
    print('pf=%f'%Pf, 'recall=%f' % Recall,'MCC=%f' % MCC)
    return skewedF1
...
def get_H1_H2_for_I_SDA(X1, X2, minSizeOfSubclass):
    ...
    while subSize*2 <= n1 and subSize*2 <= n2:
        h1 = round(n1 / subSize)
        h2 = round(n2 / subSize)
        sf = get_skewed_F_measure(X1, X2, h1, h2, 1,)
        if skewedF1 < sf:
            skewedF1 = sf
            H1 = h1
            H2 = h2
...
def get_label_of_y(X1, X2, h1, h2, y):
    ...
    X_f = np.dot(V.T, X.T)  # different with the paper to dianji
    y_f = np.dot(V.T, y.T)  # different with the paper to dianji
    # step 6
    labels1 = np.ones(n1, dtype='i4')
    labels2 = np.zeros(n2, dtype='i4')
    labels = np.concatenate((labels1, labels2), axis=0)
    rf = RandomForestClassifier()
    rf.fit(X_f.T, labels) #类标签整数化 or 用于从一个训练集中学习模型参数
